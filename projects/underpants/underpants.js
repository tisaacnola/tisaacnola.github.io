...
_.typeOf = function(value){
    return typeof(value) !== 'object' // check to see if its not an object first
        ? typeof(value)
        :Array.isArray(value)  // Then check for an array
        ? 'array'
        : value === null // check for null before object
        ? 'null'
        : value instanceof Date // check for date before object
        ? 'date'
        : 'object'; // if it not any of the above it must be an object
};
...
_.indexOf = function(array, value){
    if (array.includes(value)) { // checking to see if the value is inside the array
        for (var i = 0; i < array.length; i++) { // loop over the array
            if (value === array[i]) {  ///value is equal to object
                return i; //return
        }
     }
    } else {
...
_.contains = function(array, value){
    return array.includes(value) ? true : false; // array includes 
};
...
_.every = function(collection, func) {
    const allTrutys = [];
    if(!func) {
        _.each(collection,  e => allTrutys.push(!!e));
    } else {
     _.each(collection, function(e, i, c){
...
_.reduce = function (collection, func, seed) {
  ...
  _.each(colArray, function(e, i, c){
    if(seedUndefined) {
      seedUndefined = false;
      seed = e;
